...
func (df DataFrame) GroupBy(colnames ...string) *Groups {
	//logger.DefaultLogger.Debug("group columnas", colnames)
	if len(colnames) <= 0 {
		return nil
	}
	...
	for _, s := range df.Maps() {
		...
		for i, c := range colnames {
			...
			if s[c] == nil { // todo 这里改了，当维度数据为空的时候，以NaN填充
				format += "v"
			} else {
				switch s[c].(type) {
				case string, bool:
	...
	for k, cMaps := range groupSeries {
		//if k == "0614505546950344" {
		//	logger.DefaultLogger.Debug("cmaps", cMaps)
		//}
		groupDataFrame[k] = LoadMaps(cMaps)
	}
	//logger.DefaultLogger.Debug(
	//	"0614505546950344.Type",
	//	groupDataFrame["0614505546950344"].Col("F_163090802244776295").Type(),
	//	"0614505546950344.Val", groupDataFrame["0614505546950344"].Col("F_163090802244776295").Records(),
	//)
...
const (
	...
	Aggregation_CONCAT AggregationType = 7
)
...
type Builder interface {
	ParseDataframe(df DataFrame) DataFrame
}
