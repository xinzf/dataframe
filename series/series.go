...
func (e arrayElements) Len() int           { return len(e) }
func (e arrayElements) Elem(i int) Element { return &e[i] }
...
func New(values interface{}, t Type, name string) Series {
	...
	preAlloc := func(n int) {
		...
		case Array:
			ret.elements = make(arrayElements, n)
		case Time:
...
func Arrays(values interface{}) Series {
...
func (s *Series) Append(values interface{}) {
	...
	case Array:
		s.elements = append(s.elements.(arrayElements), news.elements.(arrayElements)...)
	}
}
...
func (s Series) Subset(indexes Indexes) Series {
	...
	case Time:
		...
		ret.elements = elements
	case Array:
		elements := make(arrayElements, len(idx))
		for k, i := range idx {
			elements[k] = s.elements.(arrayElements)[i]
		}
		ret.elements = elements
	default:
...
func (s Series) Copy() Series {
	...
	case Array:
		elements = make(arrayElements, s.Len())
		copy(elements.(arrayElements), s.elements.(arrayElements))
	}
	ret := Series{
		Name:     name,
		t:        t,
...
func (s Series) GroupConcat() string {
	if s.elements.Len() == 0 {
		return ""
	}
	...
	for i := 0; i < s.elements.Len(); i++ {
		strs = append(strs, s.elements.Elem(i).String())
	}
